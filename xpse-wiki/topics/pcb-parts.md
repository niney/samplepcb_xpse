...
| `PcbPartsMultiSearchService.java` | 자체(samplepcb) + Digikey + UniKeyIC 3개 소스 병렬 검색 (`Mono.zip`) |
...
```
[자체(samplepcb) ES]           [Digikey API]              [UniKeyIC API]
  1. keyword 완전일치            1. getProductDetails        1. searchByPartNumber
  2. 파싱 ES 검색                2. searchByKeyword 폴백      (정확매칭)
  3. partName 텍스트 검색
         |                            |                          |
