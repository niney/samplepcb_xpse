...
| 토픽 | 설명 |
|------|------|
| [PCB 부품](topics/pcb-parts.md) | 다소스 부품 데이터 수집, ES+JPA 이중 저장, 통합 검색 (Digikey, UniKeyIC, IC114) |
| [PCB 검색 인덱스](topics/pcb-search-index.md) | Kind/Column/Item/Unit 분류 체계, Elasticsearch 인덱스 관리, 벡터 유사도 검색 |
| [견적 관리](topics/sp-estimate.md) | 견적서 생성, 협력사 견적 제출/선택, 주문 확정 업무 흐름 |
| [BOM 문서 관리](topics/sp-bom.md) | BOM 문서 회원별 CRUD, contentHash 기반 upsert |
| [주문 관리](topics/sp-order.md) | G5 쇼핑몰 주문 조회, 협력사 발주 CRUD |

### 외부 연동

| 토픽 | 설명 |
|------|------|
| [외부 부품 API 연동](topics/external-integration.md) | Digi-Key, UniKeyIC, IC114, Google USE 연동, 병렬 검색, 이중 저장 |

### 인프라

| 토픽 | 설명 |
|------|------|
| [인증/인가](topics/security.md) | JWT 검증 전용 무상태 인증, opt-in `@JwtAuth` 어노테이션 모델 |
| [설정/인프라](topics/infrastructure.md) | Elasticsearch, MySQL, Caffeine Cache, QueryDSL, Spring Security 설정 |
